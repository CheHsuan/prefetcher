...
void sse_transpose_omp(int *src, int *dst, int w, int h)
{
    #pragma omp parallel num_threads(THREADS)
    {
        #pragma omp for
...
void sse_prefetch_transpose_omp(int *src, int *dst, int w, int h)
{
    #pragma omp parallel num_threads(THREADS)
    {
        #pragma omp for
...
void sse_prefetch_transpose_2(int *src, int *dst, int w, int h, int pfdist)
{
    for (int x = 0; x < w; x += 4) {
        for (int y = 0; y < h; y += 4) {
